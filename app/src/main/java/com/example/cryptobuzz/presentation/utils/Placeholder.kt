...
fun Modifier.placeholder(
    ...
    showShimmerAnimation: Boolean = false
): Modifier = composed {
    val highlight = if (showShimmerAnimation) {
        PlaceholderHighlight.shimmer(highlightColor = Color.LightGray.copy(.5f))
    } else {
        PlaceholderHighlight.fade(highlightColor = Color.LightGray.copy(.5f))
    }
    val specifiedBackgroundColor = backgroundColor.takeOrElse { LocalContentColor.current.copy(.5f) }
